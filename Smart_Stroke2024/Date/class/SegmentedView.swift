//
//  SegmentedView.swift
//  KSR
//
//  Created by m2comm on 2018. 6. 21..
//  Copyright © 2018년 m2community. All rights reserved.
//

import UIKit

@objc protocol SegmentedViewDelegate {
    @objc optional func segButtonPressed(index : Int)
}

class SegmentedView: UIView {

    var delegate : SegmentedViewDelegate?

    var itemButtons = [UIButton]()
    
    var buttonsBackView : UIView!
    
    var buttonRatios : [CGFloat] = [1/3, 1/3, 1/3]
    
    
    convenience init(ratios : [CGFloat] = [1/3, 1/3, 1/3]) {
        self.init(frame: CGRect(x: 15, y: 0, width: SCREEN.WIDTH - 30, height: 50), ratios: ratios)
    }
    
    init(frame: CGRect, ratios : [CGFloat]) {
        super.init(frame: frame)
     
        self.buttonRatios = ratios
        
        self.backgroundColor = UIColor.white
//        self.layer.borderWidth = 1
        let leftMargin : CGFloat = 20
        let topMargin : CGFloat = 10
        self.buttonsBackView = UIView(frame: CGRect(x: leftMargin, y: topMargin, width: SCREEN.WIDTH - (leftMargin * 2), height: self.frame.size.height - (topMargin * 2)))
        self.addSubview(self.buttonsBackView)
        
    }
    
    let buttonFontSize : CGFloat = 8
    func addItems( titles : [String]){
        
        var lastX : CGFloat = 0
        for i in 0..<titles.count {
            let buttonWidth = self.buttonsBackView.frame.size.width * self.buttonRatios[i]
//            let button = UIButton(frame: CGRect(x: lastX + 15, y: 0, width: buttonWidth - 30, height: self.buttonsBackView.frame.size.height))
            let button = UIButton(frame: CGRect(x: lastX + 55, y: 0, width: buttonWidth - 30, height: self.buttonsBackView.frame.size.height))
            button.setTitle(titles[i], for: .normal)
            button.tag = i
            
//            button.layer.borderWidth = 1
            button.layer.cornerRadius = 14
            button.setTitleColor(UIColor.black, for: .normal)
            button.backgroundColor = #colorLiteral(red: 0.9489794374, green: 0.960236609, blue: 0.9856713414, alpha: 1)
//            button.titleLabel?.font = UIFont(name: "SUITE-Regular", size: IS_IPHONE_SE ? 13 : 15)
            button.titleLabel?.font = UIFont(name: "SUITE-Regular", size: 8)
//            button.nameLabel.font = UIFont(name: "SUITE-Regular", size: 20)
            
            
            button.addTarget(self, action: #selector(itemButtonPressed(button:)), for: .touchUpInside)
            self.itemButtons.append(button)
            self.buttonsBackView.addSubview(button)
            
            
            
            if i != 0 {
                let separatorView = UIView(frame: CGRect(x: lastX, y: 0, width: 0.5, height: self.buttonsBackView.frame.size.height * 0.5))
                if self.buttonRatios[0] == 0.3 {
                    if i == 1 {
                        button.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: 8)
                        separatorView.center.x = self.frame.size.width * 0.26
                    }else{
                        button.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: 8)
                        separatorView.center.x = self.frame.size.width * 0.7
                    }
                }
                
                separatorView.backgroundColor = UIColor(white: 0.69, alpha: 1)
                separatorView.center.y = self.buttonsBackView.frame.size.height / 2
                self.buttonsBackView.addSubview(separatorView)
            }
            lastX = button.frame.maxX
            
            if i == 0 {
//                button.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: IS_IPHONE_SE ? 13 : 15)
                button.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: 8)
                button.setTitleColor(#colorLiteral(red: 0.3855227828, green: 0.4413648248, blue: 0.5808759332, alpha: 1), for: .normal)
            } else {
                button.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: 8)
                button.setTitleColor(#colorLiteral(red: 0.3855227828, green: 0.4413648248, blue: 0.5808759332, alpha: 1), for: .normal)
            }
        }
        
    }
    
    
    
    //세그먼트 액션
    @objc func itemButtonPressed( button : UIButton ){
        print("sdasdsad")
        for itemButton in self.itemButtons {
            if button == itemButton {
//                itemButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: IS_IPHONE_SE ? 13 : 13)
                itemButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: 8)
                itemButton.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
                itemButton.layer.borderWidth = 1
                itemButton.layer.borderColor = UIColor(red:222/255, green:225/255, blue:227/255, alpha: 1).cgColor
                itemButton.setTitleColor(#colorLiteral(red: 0.1708087921, green: 0.5327006578, blue: 0.8681024909, alpha: 1), for: .normal)
                
            }else{
//                itemButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Ultra_Light, size: IS_IPHONE_SE ? 13 : 13)
                itemButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: 8)
                itemButton.setTitleColor(.lightGray, for: .normal)
                itemButton.backgroundColor = #colorLiteral(red: 0.9187068343, green: 0.9332650304, blue: 0.9892967343, alpha: 1)
                itemButton.layer.borderWidth = 0
            }
        }
        
        self.delegate?.segButtonPressed?(index: button.tag)
    }
    
    required init?(coder aDecoder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
}
