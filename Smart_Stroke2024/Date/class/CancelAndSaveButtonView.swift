...
class CancelAndSaveButtonView: UIView{
    ...
    convenience init(type :  ButtonType ) {
        self.init(frame: CGRect(x: 0, y: 20, width: SCREEN.WIDTH, height: 80), type : type)
    }
    ...
    private init(frame: CGRect, type : ButtonType) {
        ...
        if type == .notFull {
            ...
            self.cancelButton = UIButton(frame: CGRect(x: 15, y: 0, width: buttonWidth, height: 80))
            ...
            self.saveButton.backgroundColor = #colorLiteral(red: 0.614526093, green: 0.6657052636, blue: 0.7210657001, alpha: 1)
//            self.cancelButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF, size: 13)
            self.cancelButton.setTitle("취소", for: .normal)
            self.cancelButton.titleLabel?.font = UIFont(name: "SUITE-Heavy", size: 20)
            self.cancelButton.addTarget(self, action: #selector(cancelButtonPressed(button:)), for: .touchUpInside)
            self.addSubview(self.cancelButton)
            
            self.saveButton = UIButton(frame: CGRect(x: 0, y: 0, width: buttonWidth, height: 80))
            ...
            self.saveButton.backgroundColor = #colorLiteral(red: 0.1406538188, green: 0.347869575, blue: 0.5851272941, alpha: 1)
...
//            self.saveButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF, size: 13)
            ...
            self.saveButton.titleLabel?.font = UIFont(name: "SUITE-Heavy", size: 20)
            self.saveButton.addTarget(self, action: #selector(saveButtonPressed(button:)), for: .touchUpInside)
            self.addSubview(self.saveButton)
        }else{
            self.cancelButton = UIButton(frame: CGRect(x: 0, y: 0, width: SCREEN.WIDTH / 2, height: 80))
//            self.cancelButton.backgroundColor = UIColor(red: 59/255, green: 77/255, blue: 91/255, alpha: 1)
            self.cancelButton.backgroundColor = #colorLiteral(red: 0.614526093, green: 0.6657052636, blue: 0.7210657001, alpha: 1)
            self.cancelButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF, size: 13)
            self.cancelButton.setTitle("취소", for: .normal)
            self.cancelButton.titleLabel?.font = UIFont(name: "SUITE-Heavy", size: 20)
            self.cancelButton.addTarget(self, action: #selector(cancelButtonPressed(button:)), for: .touchUpInside)
...
//            cancelButton.addSubview(leftImageView)
            ...
            self.saveButton = UIButton(frame: CGRect(x: SCREEN.WIDTH / 2, y: 0, width: SCREEN.WIDTH / 2, height: 80))
//            self.saveButton.backgroundColor = UIColor(red: 84/255, green: 153/255, blue: 244/255, alpha: 1)
            self.saveButton.backgroundColor = #colorLiteral(red: 0.1406538188, green: 0.347869575, blue: 0.5851272941, alpha: 1)
            self.saveButton.titleLabel?.font = UIFont(name: Nanum_Barun_Gothic_OTF, size: 13)
            self.saveButton.setTitle("저장", for: .normal)
