...
class AlramCenter: NSObject,UNUserNotificationCenterDelegate {
    
    static let shared : AlramCenter = {
        let sharedCenter = AlramCenter()
        return sharedCenter
    }()
    ...
    //알람 추가
    func addAlramToNoti( addDate : Date = Date(), info : AlramInfo, complete: @escaping (_ success : Bool)->Void ){
        ...
        if !info.isAlram {
            ...
            //알람 제거
            self.removeAlramFromNoti(info: info)
            
            //save
            self.saveAlramList(info: info)
        ...
        let dbInfo = AlramDB(fileName: info.alramType.rawValue, registerDate: registerDate, isBefore: info.isBefore, hour: info.hour, minute: info.minute, drugs: info.drugs)
        ...
        notiCenter.add(request) { (error : Error?) in
            if error == nil {
                //성공하면 DB에 저장
                self.saveAlramList(info: info)
                complete(self.saveAlramToJson(info: dbInfo))
            }else{
                complete(false)
            }
        }
...
//        let alramId = info.alramType.rawValue
        let alramId = info.isAlram
        
        print("alramId:\(alramId)")
