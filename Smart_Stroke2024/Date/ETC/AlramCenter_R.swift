...
//            notiCenter.removePendingNotificationRequests(withIdentifiers: [alram.alramKey])
            ...
            notiCenter.removeDeliveredNotifications(withIdentifiers: ["\(alram.idx)"])
            notiCenter.removePendingNotificationRequests(withIdentifiers: ["\(alram.idx)"])
            complete(true)
        }else{
            print("알람 추가")
            ...
            let request = UNNotificationRequest(identifier: "\(alram.idx)", content: content, trigger: trigger)
